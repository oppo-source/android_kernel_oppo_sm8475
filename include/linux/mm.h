...
#define chp_log(l, f, ...) do {						\
	if (l >= CHP_LOG_LVL) 						\
		printk(KERN_ERR "%s %5d %c %-16s: %s:%d "f,		\
		       CHP_TAG, current->tgid, chp_loglvl_to_char(l),	\
...
#define VM_FLAG_CHP VM_NOHUGEPAGE
...
extern unsigned long double_mapped_swap_cluster;
...
{
	return !!(vma->vm_flags & VM_FLAG_CHP);
}
...
#else
static inline free_zram_is_ok(void) { return true; }
#endif
