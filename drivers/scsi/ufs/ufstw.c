...
static int ufstw_write_lu_attr(struct ufstw_lu *tw, u8 idn, u32 *val)
{
	...
	lun = (tw->lun == TW_LU_SHARED) ? 0 : tw->lun;
	err = ufshcd_query_attr_retry(hba, UPIU_QUERY_OPCODE_WRITE_ATTR, idn,
...
static void ufstw_update_resized_buffer(struct ufstw_lu *tw, u32 resize_status)
{
	...
	if (resize_status == UFSTW_RESIZE_STATUS_SUCCESS)
		...
		TW_DEBUG(tw, "resize operation is failed");
...
static bool ufstw_is_resize_needed(struct ufstw_lu *tw)
{
	...
	if (full_count) {
		...
		if (tw->resize_fully_increased ||
		    tw->curr_buffer_size >= tw->lu_buf_size) {
			TW_DEBUG(tw, "buffer is fully increased, clear full_count");
...
static int ufstw_lu_init(struct ufsf_feature *ufsf, int lun)
{
	...
	ufstw_set_lu_flag(tw, QUERY_FLAG_IDN_TW_FLUSH_DURING_HIBERN,
			  &tw->flush_during_hibern_enter);
	ufstw_set_lu_flag(tw, QUERY_FLAG_IDN_TW_BUF_FULL_COUNT_INIT,
			  &flag_res);
							  ...
							  size_t count)
{
	unsigned long val;
	ssize_t ret;
	struct ufstw_dev_info *tw_dev_info = &tw->ufsf->tw_dev_info;

	if (kstrtoul(buf, 0, &val))
		return -EINVAL;

	if (tw_dev_info->tw_buf_type == TW_BUF_TYPE_LU &&
	    val > (unsigned long)tw->lu_buf_size)
		return -EINVAL;

	else if (tw_dev_info->tw_buf_type == TW_BUF_TYPE_SHARED &&
		 val > (unsigned long)tw_dev_info->tw_shared_buf_alloc_units)
		return -EINVAL;
	...
	INFO_MSG("max_no_flush_alloc_units(%lu) write query success", val);
...
static ssize_t ufstw_sysfs_show_resize_status(struct ufstw_lu *tw, char *buf)
{
	...
	INFO_MSG("resize_auto: %u (%s) resize_status: %u (%s)",
