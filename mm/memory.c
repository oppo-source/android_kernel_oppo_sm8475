...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	if (is_zero_pfn(pte_pfn(vmf->orig_pte))) {
		...
		if (!new_page)
			goto out;
#ifdef CONFIG_CONT_PTE_HUGEPAGE
		BUG_ON(PageCont(new_page));
		BUG_ON(PageContRefill(new_page));
#endif
	} else {
#ifdef CONFIG_CONT_PTE_HUGEPAGE
		...
		if (old_page) {
			if (!PageAnon(old_page)) {
				...
				if (!pte_cont(vmf->orig_pte)) {
					...
					BUG_ON(!is_huge_zero_page(old_page));
...
static bool wp_cont_pte_hugepage_reuse(struct vm_fault *vmf)
{
	...
	if (reuse_swap_cont_pte_page(head, NULL)) {
		pte_t entry;

		/* we reuse hugepage as a whole */
		BUG_ON(!cont_pte_readonly(vmf));
...
out:
	BUG_ON(ret && (ci->flags & CLUSTER_FLAG_FREE));
	unlock_cluster(ci);
	return ret;
}
...
static int cont_pte_swapcache_prepare(swp_entry_t *pentry)
{
	...
	BUG_ON(!IS_ALIGNED(offset, HPAGE_CONT_PTE_NR));
	...
	if (!page) {
		...
		if (data_race(si->flags & SWP_SYNCHRONOUS_IO) &&
			...
			if (is_thp_swap(si)) {
				...
				if (suitable) {
					pr_err("@@@@%s-%d current:%s-%d vma:%llx-%llx fault addr:%llx pte-unchanged doublemap eligible:%d\n",
						...
						vma_is_chp_anonymous(vma));
					for (i = 0; i < HPAGE_CONT_PTE_NR; i++) {
						pr_err("@@@ i:%d pte none:%d pte present:%d\n", i, pte_none(*(ptep + i)), pte_present(*(ptep + i)));
					}
				}
			}
#endif
			...
			page = swapin_readahead(entry, GFP_HIGHUSER_MOVABLE,
						vmf);
#ifdef CONFIG_CONT_PTE_HUGEPAGE
			BUG_ON(page && PageCont(page));
			BUG_ON(page && PageContRefill(page));
#endif
			}
			swapcache = page;
		}
