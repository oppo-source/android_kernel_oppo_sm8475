				     ...
				     bool ignore_references)
{
	...
	while (!list_empty(page_list)) {
		...
		BUG_ON(sc->gfp_mask & POOL_USER_ALLOC_MASK && !ContPteCMAHugePageHead(page));
#endif
		...
		enum lru_list lru)
{
	...
	while (scan < nr_to_scan && !list_empty(src)) {
		...
		BUG_ON(chp_reclaim && sc->order != HPAGE_CONT_PTE_ORDER);
#endif
			   ...
			   unsigned long *nr)
{
	...
	bool free_zram_ok = free_zram_is_ok();


	/* If we have no swap space, do not bother scanning anon pages. */
	if (!chp_reclaim && !free_zram_ok) {
