...
#ifdef CONFIG_CONT_PTE_HUGEPAGE
	if (is_thp_swap(p)) {
		BUG_ON(!(inode && S_ISBLK(inode->i_mode) && p->bdev && p->bdev->bd_disk));
#ifdef CONFIG_CONT_PTE_HUGEPAGE_64K_ZRAM
		/* we hard code zram1 as hugepage zram in zram driver */
		BUG_ON(p->bdev->bd_disk->first_minor != 1);
#endif
		p->bdev->bd_disk->android_kabi_reserved1 = THP_SWAP_PRIO_MAGIC;
		pr_info("swapon No.%d thp swap\n", p->type);
	}else {
		p->bdev->bd_disk->android_kabi_reserved1 = &g_cont_pte_pool;
		pr_info("swapon No.%d page swap\n", p->type);
	}
