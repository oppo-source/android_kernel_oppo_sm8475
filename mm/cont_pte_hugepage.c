...
/* set anon_enable to 1 after we fix android boot */
static bool cont_pte_anon_enable = true;
module_param_named(cont_pte_anon_enable, cont_pte_anon_enable, bool, S_IRUGO | S_IWUSR);

static bool cont_pte_alloc_oom = false;
module_param_named(cont_pte_alloc_oom, cont_pte_alloc_oom, bool, S_IRUGO | S_IWUSR);
...
unsigned long double_mapped_swap_cluster;
...
inline bool current_is_hybridswapd()
{
...
static void huge_page_pool_add(struct huge_page_pool *pool, struct page *page, int inx)
{
	...
	pool->count[inx]++;
	mod_node_page_state(page_pgdat(page), NR_KERNEL_MISC_RECLAIMABLE,1 << HPAGE_CONT_PTE_ORDER);
				 ...
				 unsigned long len)
{
	...
	unsigned int native_addr = rsv & UINT_MAX;
	unsigned int dalvik_addr = (rsv >> 32);
	unsigned int code = addr & UINT_MAX;
	...
	if (native_addr && dalvik_addr &&
	    code != native_addr && code != dalvik_addr)
		return false;

	for (; i < MAX_LEN_CHP_VMA_NAME && !copy_from_user(&c, name + i, 1); i++) {

		if (i == 0) {
			switch (c) {
			case 'd':
				expect_buf = DALVIK_MAIN_HEAP;
				break;
			case 'l':
				expect_buf = NATIVE_HEAP;
				break;
			default:
				return false;
			}
		...
		if (c == '\0') {
			...
			case 'l':
				if (!native_addr)
					current->mm->android_kabi_reserved1 |= code;
				break;
			case 'd':
				if (!dalvik_addr)
					current->mm->android_kabi_reserved1 |= ((u64)code) << 32;
				break;
			}
			return true;
		}
...
static inline int current_ux_task(void)
{
	bool is_ux = false;
	 ...
	 */
	trace_android_vh_pcplist_add_cma_pages_bypass(0,
		&is_ux);
...
static inline unsigned int get_pool_alloc_flags(gfp_t gfp_mask)
{
	...
	if (rt_task(tsk) || current_ux_task() || (gfp_mask & __GFP_HIGH))
			      ...
			      pte_t *ptep)
{
	...
	BUG_ON (!pte_cont(READ_ONCE(*ptep)));
...
#define THP_SPLIT_CONT_PTE THP_SPLIT_PMD       /* we are leveraging PMD count for CONT_PTE */
	...
	/* FIXME: need pte invalidate and flush tlb ??? */
	for (i = 0; i < HPAGE_CONT_PTE_NR; i++, haddr += PAGE_SIZE, pte++) {
		if (freeze) {
...
repeat:
	if (pte_cont(*pte)) {
		...
		BUG_ON(!cont_pte_trans_huge(pte));
...
#if 0
		/* unlike__split_huge_pmd, we never map dev as cont_pte */
		if (pte_devmap(*pte));
#endif
		/*
...
static int proc_stat_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "double_mapped_swap_cluster %lu\n",
		   double_mapped_swap_cluster);
...
static int proc_csv_stat_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,%10d,%10lu,%10lu\n",
		   ...
		   read_zram_used_pages(0),
		   read_zram_used_pages(1),
#else
		   0,
		   0,
#endif
...
static int __init cmdline_parse_prjname(char *p)
{
	static const char *prjs[] = {
		"22803", "22881", "21001", "21201", "22091", "22227", NULL,
	};
	int i = 0;
